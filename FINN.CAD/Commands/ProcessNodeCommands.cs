...
namespace FINN.CAD.Commands
{
	public class ProcessNodeCommand
	{
		...
		internal static Models.ProcessNode PromptInput()
		{
			...
			if (r1.Status != PromptStatus.OK) return null;
			...
			return new Models.ProcessNode()
			{
				...
				Position = new Point2d(r1.Value.X < r2.Value.X ? r1.Value.X : r2.Value.X, r1.Value.Y > r2.Value.Y ? r1.Value.Y : r2.Value.Y),
			};
		}

		internal static ObjectId InnerExecute()
		{
